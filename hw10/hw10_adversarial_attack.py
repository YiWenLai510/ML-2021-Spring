...
def fgsm(models, x, y, loss_fn, epsilon=epsilon):
    ...
    for model in models:
        model.eval()
        yp += model.to(device)(x_adv)
    yp = yp / len(models)
    # print(yp.size())
    # yp = torch.argmax(yp,dim=1)

    loss = loss_fn(yp, y) # calculate loss
...
def ifgsm(models, x, y, loss_fn, epsilon=epsilon, alpha=alpha, num_iter=20):
    ...
    for i in range(num_iter):
        ...
        x_adv, yp = fgsm(models, x_adv, y, loss_fn, alpha)        
...
def gen_adv_examples(models, loader, attack, loss_fn):
    ...
    for i, (x, y) in enumerate(loader):
        x, y = x.to(device), y.to(device)
        x_adv, yp = attack(models, x, y, loss_fn) # obtain adversarial examples
        # yp = model(x_adv)
        loss = loss_fn(yp, y)
        train_acc += (yp.argmax(dim=1) == y).sum().item()
        train_loss += loss.item() * x.shape[0]
...
model_list = ['resnet110_cifar10', 'preresnet20_cifar10', 'seresnet20_cifar10', 'diaresnet20_cifar10', 'densenet40_k12_cifar10']
models = []
for modelName in model_list:
    models.append(ptcv_get_model(modelName, pretrained=True))
...
create_dir(root, 'ifgsm_ensemble5', adv_examples, adv_names)
